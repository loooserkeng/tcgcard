...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [editBeautyRate, setEditBeautyRate] = useState(95);
  ...
  const filteredCards = useMemo(() => {
    return cards.filter((card) => {
      const categoryStr = card.category?.toLowerCase() || '';
      const matchesSearch =
  ...
  const handleStartEdit = (card: PersonCard) => {
    ...
    setEditDob(card.dateOfBirth || '');
    setEditAge(card.age);
    setEditBeautyRate(card.beautyRate || card.stats?.charisma || 92);
    setEditMovies(card.movies ? card.movies.join(', ') : '');
    setEditPhoto(card.photo || card.image || '');
    setEditRarity(card.rarity);
    setEditCategory(card.category || 'CINEMA');
    setEditDescription(card.description || '');
    setEditCardNumber(card.cardNumber);
    setEditCharisma(card.stats?.charisma || 85);
    setEditEnergy(card.stats?.energy || 85);
    setEditStyle(card.stats?.style || 85);
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    const updatedCard: PersonCard = {
      ...editingCard,
      name: editName.trim(),
      dateOfBirth: editDob.trim() || editingCard.dateOfBirth,
      age: Number(editAge),
      beautyRate: Number(editBeautyRate),
      movies: parsedMovies.length > 0 ? parsedMovies : editingCard.movies,
      photo: editPhoto,
      rarity: editRarity,
      ...
      stats: {
        charisma: Number(editBeautyRate),
      ...
      {activeSection === 'CARDS' && (
        <>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-mono font-bold text-xs flex items-center gap-2 cursor-pointer shadow-lg shadow-red-600/30 transition-all border border-red-400/30"
              >
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-3.5 py-2 rounded-xl bg-neutral-950/70 border border-white/15 text-white placeholder:text-neutral-500 text-xs font-mono focus:outline-none focus:border-red-500"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-3 mb-4">
            {['ALL', 'COMMON', 'RARE', 'SPECIAL', 'EPIC', 'LEGENDARY'].map((r) => (
              <button
                key={r}
          ...
          <div className="rounded-2xl bg-neutral-950/70 border border-white/10 overflow-hidden shadow-xl">
            <div className="p-4 border-b border-white/10 flex items-center justify-between text-xs font-mono text-neutral-400">
              ...
              <button
                ...
                className="flex items-center gap-1 text-red-400 hover:text-red-300 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                        ...
                        <span
                          className={`text-[10px] font-mono font-bold tracking-wider uppercase ${
                            card.rarity === 'LEGENDARY'
                              ? 'text-amber-400'
                              : card.rarity === 'ULTRA_RARE' || card.rarity === 'EPIC'
                              ? 'text-rose-400'
                              : card.rarity === 'SPECIAL'
                              ? 'text-amber-300'
                              : card.rarity === 'RARE'
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-white/5 transition-colors">
                        <td className="p-3.5 font-bold text-white flex items-center gap-2">
                          <div className="w-6 h-6 rounded-md bg-gradient-to-tr from-red-600 to-rose-600 flex items-center justify-center text-[10px] font-black">
            ...
            <form onSubmit={handleSaveEdit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-white/5 border border-white/15 text-xs text-white focus:outline-none focus:border-red-500"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-neutral-900 border border-white/15 text-xs text-white focus:outline-none focus:border-red-500"
                >
              ...
              <div className="sm:col-span-2">
                <label className="block text-[11px] font-mono text-neutral-300 font-bold mb-1">
                  FAMOUS MOVIES (COMMA-SEPARATED)
                </label>
                <input
                  type="text"
                  value={editMovies}
                  onChange={(e) => setEditMovies(e.target.value)}
                  placeholder="e.g. Barbie, Babylon"
                  className="w-full px-3.5 py-2 rounded-xl bg-white/5 border border-white/15 text-xs text-white focus:outline-none focus:border-red-500"
                />
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    min="70"
                    max="100"
                    value={editBeautyRate}
                    onChange={(e) => setEditBeautyRate(Number(e.target.value))}
                    className="flex-1 accent-red-500"
                  />
                  <span className="font-mono text-amber-400 font-bold text-xs w-12 text-right">
                    {editBeautyRate} / 100
                  </span>
                </div>
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2 rounded-xl bg-white/5 border border-white/15 text-xs text-white focus:outline-none focus:border-red-500"
                  />
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-white/5 border border-white/15 text-xs text-white focus:outline-none focus:border-red-500 font-sans"
                />
              </div>
              ...
              <div className="sm:col-span-2 flex justify-end gap-2 pt-4 border-t border-white/10">
                ...
                <button
                  type="submit"
                  disabled={savingEdit}
                  className="px-5 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-mono font-bold uppercase tracking-wider cursor-pointer disabled:opacity-50 shadow-lg shadow-red-600/30"
                >
