...
interface CardRevealProps {
  ...
  onFinishPack: (revealedCards: ActressCard[]) => void;
...
export const CardReveal: React.FC<CardRevealProps> = ({
  ...
  const isHighRarity = ['RARE', 'SPECIAL', 'EPIC', 'ULTRA_RARE', 'LEGENDARY'].includes(currentCard.rarity);
        ...
        try {
          if (isLegendary) {
            ...
            confetti({
              ...
              spread: 80,
              origin: { y: 0.6 },
              colors: [rarityInfo.color, '#ff2e5b', '#ffffff'],
            });
          }
        } catch {}
      }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-30 w-full max-w-lg flex items-center justify-between">
        {/* Progress pill */}
        <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-red-950/60 backdrop-blur-md border border-red-500/30 text-xs font-mono font-bold text-red-200">
          ...
          {cards.map((_, idx) => (
            <div
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                idx === currentIndex
                  ? 'w-6 bg-red-500 shadow-sm shadow-red-500/80'
                  : revealedMap[idx]
                  ? 'w-2 bg-red-400'
                  : 'w-2 bg-red-950 border border-red-900'
              }`}
            />
          ))}
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-red-950/50 hover:bg-red-900/50 border border-red-500/30 text-xs font-mono text-red-200 transition-all cursor-pointer"
        >
      ...
      <div className="relative z-30 my-auto flex flex-col items-center justify-center">
        ...
        <AnimatePresence>
          {specialEffectActive && isLegendary && (
            <motion.div
              ...
              className="absolute -top-14 z-50 flex items-center gap-2 px-6 py-2 rounded-full bg-gradient-to-r from-amber-500 via-red-600 to-amber-500 text-white font-black font-serif tracking-[0.25em] text-sm uppercase shadow-2xl shadow-amber-400/80 border-2 border-amber-300"
            >
              <Crown className="w-4 h-4 fill-white" />
              <span>LEGENDARY ACTRESS!</span>
              <Crown className="w-4 h-4 fill-white" />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        >
          ...
          >
            ...
            >
              <div className="w-72 h-[460px] sm:w-80 sm:h-[510px]">
                <CardBack />
              </div>
            </div>
          </motion.div>
        ...
        <div className="mt-4 text-center">
          {!isCurrentRevealed ? (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-gradient-to-r from-red-600 via-rose-500 to-red-600 hover:from-red-500 hover:to-rose-400 text-white font-extrabold text-sm font-serif tracking-widest uppercase shadow-lg shadow-red-600/40 cursor-pointer"
            >
            ...
            <div className="flex items-center gap-3">
              {!isAlreadyCollected ? (
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-500/20 border border-red-400/40 text-red-200 text-xs font-mono font-bold tracking-wider">
                  <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                  <span>NEW DISCOVERY ADDED TO COLLECTION</span>
                </div>
              ) : (
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/60 border border-red-500/40 text-red-300 text-xs font-mono font-bold tracking-wider">
      ...
      <div className="relative z-30 w-full max-w-lg flex items-center justify-between pt-2">
        ...
        <button
          ...
          className={`flex items-center gap-1 px-4 py-2 rounded-xl text-xs font-mono font-bold tracking-wider transition-all ${
            currentIndex === 0
              ? 'opacity-30 cursor-not-allowed bg-red-950/20 text-red-700'
              : 'bg-red-950/50 hover:bg-red-900/60 text-red-200 cursor-pointer border border-red-500/30'
          }`}
        >
        ...
        {!isCurrentRevealed ? (
          <button
            ...
            onClick={triggerReveal}
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-extrabold text-xs font-mono tracking-widest uppercase transition-all shadow-lg shadow-red-600/40 cursor-pointer"
          >
            <span>REVEAL</span>
            <Eye className="w-4 h-4" />
          </button>
          ...
          <button
            id="reveal-next-btn"
            onClick={handleNext}
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-black text-xs font-mono tracking-widest uppercase transition-all shadow-lg shadow-red-600/40 cursor-pointer"
          >
